...
$app->post('/CheckAuth', function () {
    global $DB, $app;
    if (isset($_POST['AuthToken'], $_POST['username']) && !empty($_POST['AuthToken']) && !empty($_POST['username'])) {
        $Auth = $_POST['AuthToken'];
        $username = $_POST['username'];

        $user_exist_qry = $DB->query("SELECT `u_id` FROM `" . TAB_MEMBER . "` WHERE `email`='{$username}'") or iDie($app, $DB->error);


        if ($user_exist_qry->num_rows ==1) {

            $qry = $DB->query("SELECT `u_id`,`isPremium`,`expiration_date`,`authorization_token` FROM `" . TAB_MEMBER . "` as mem INNER JOIN `" . TAB_SUBSCRIPTION . "` as sub WHERE mem.`u_id`=sub.`mem_id` AND `expiration_date` > CURDATE() AND mem.`email`='{$username}'") or iDie($app, $DB->error);
            ...
            if ($qry->num_rows > 0)
            {
                ...
                $expire_date = $info['expiration_date'];

                $old_token = $info['authorization_token'];


                if ($old_token != $Auth) {
                    $result['status'] = 'no';
                    $result['result'] = 'This account is used by another devices';
                    $result['logout'] = true;
                } else {
            ...
            $result['status'] = 'no';
            $result['result'] = 'Account doesn\'t exist';
            $result['logout'] = true;
        }

    } else {
        $result['status'] = 'no';
        $result['result'] = 'Required parameter is missing';
    }
